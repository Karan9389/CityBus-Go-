...
export default function DriverLogin({ onShowScreen, onGoBack, onDriverLogin, onShowNotification, onGoHome }: DriverLoginProps) {
 ...
 const onSubmit = async (data: any) => {
  try {
    ...
    if (response.ok) {
      // Save the JWT token to the browser to keep them logged in
      localStorage.setItem('driver_token', result.token);
      
      onShowNotification("Login successful! Welcome back.");
      
      // Pass the real database driver object up to App.tsx
      onDriverLogin({
        id: result.driver.id,
        name: result.driver.name,
        phone: result.driver.phone,
        password: '' // Don't store the password in frontend state
      });
      
      onShowScreen('driverDashboard');
    } else {
      onShowNotification(result.message || "Invalid credentials.");
    }
  } catch (error) {
    console.error("Login error:", error);
    onShowNotification("Cannot connect to the server.");
  }
};
